...
describe(`Test of 'ZeroArgument.js', n=${n}, m=${m}`, function () {
    ...
    it("Test of the witness", () => {
        ...
        tmp = tmp.fromRed();
        assert.isTrue(tmp.eq(new BN(0)));
        for (let i = 0; i < m; i++) {
            assert.isTrue(Commitment_exec.isEqual(c_A_vector[i], PedersenPublicKey_exec.commit(ec, ck, A_matrix_T[i], r_vector[i])[0]));
            assert.isTrue(Commitment_exec.isEqual(c_B_vector[i], PedersenPublicKey_exec.commit(ec, ck, B_matrix_T[i], s_vector[i])[0]));
        }
    });
